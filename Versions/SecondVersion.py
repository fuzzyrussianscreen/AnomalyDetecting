...
def printDF(df):
	...
	# dfNames = pd.DataFrame(pd.unique(df['Vul']))
	...
	for n in range(1):
		...
		df_subset = SearchAnomaly(df[["value"]], axex)
		# df_subset = []
		# print(df_subset)
		# plt.subplots_adjust(wspace=0, hspace=i)
		df[["value"]].plot(ax=axex[0], legend=True, color="black")
		df[["value"]].plot(ax=axex[1], legend=True, color="black")
		df[["value"]].plot(ax=axex[2], legend=True, color="black")
		df_subset = df.loc[df_subset.index]
		if len(df_subset) > 0:
			df_subset[["value"]].plot(ax=axex[0], legend=False, color="r", marker='s', linewidth=0)
			df_subset[["value"]].plot(ax=axex[1], legend=False, color="r", marker='s', linewidth=0)
			df_subset[["value"]].plot(ax=axex[2], legend=False, color="r", marker='s', linewidth=0)
...
with open('../Sources/data_ver2.csv', newline='') as csvfile:
	...
	dfSource['value'] = dfSource['value'].astype(float)
	print(dfSource)
